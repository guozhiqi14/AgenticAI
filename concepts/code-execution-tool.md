...
## Code Execution As Planning

在 Module 5 的 planning 语境里，code execution 不只是一个“计算工具”，也可以是一种 plan 表达方式。

```text
LLM writes code
  -> code calls library functions in a useful order
  -> sandbox executes the code
  -> result/error returns
```

这叫 code-as-plan：plan 不再是单独的 JSON step list，而是写在代码的函数调用、变量、中间结果、循环和条件分支里。

对 pandas/CSV 分析尤其有用。与其为 `filter_rows`、`group_by_month`、`count_unique`、`latest_rows` 等能力逐个封装工具，不如在 sandbox 中让 LLM 写 pandas 代码，并对代码、结果和权限做约束。
...
- 代码能运行但逻辑错误，最终答案看似合理但其实口径错。
- 把高风险业务动作也交给自由代码执行，绕过 tool permissions。

## Related Notes

- [Code Execution](../notes/module-03-tool-use/06-code-execution.md)
- [Planning with code execution](../notes/module-05-patterns-for-highly-autonomous-agents/03-planning-with-code-execution.md)
