...
4. 单独运行这个 component。
5. 根据 component 类型选择修法：non-LLM component 先试 parameters / provider / tool replacement；LLM component 先试 prompt / model / step split / fine-tuning。
6. 比较不同 prompt / model / tool / parameter 的 component score。
7. 局部优化完成后，再跑 end-to-end eval 验证整体是否变好。
...
如果优化目标是 latency/cost，也可以把 component-level eval 变成质量 + 性能的组合检查：例如 SQL generation 不只看 SQL 是否正确，也看 token cost、生成时间和是否需要 retry。
...
- 这是 non-LLM component 的参数/工具问题，还是 LLM component 的 prompt/model/任务拆解问题？
- 如果是 latency/cost 问题，这个 component 是否真的是最大 contributor？
...
- 发现 component 错误后只改 prompt，但真正瓶颈在 retrieval、search、parser、threshold 或数据质量。
- Gold standard 太窄，导致只优化少数样例。
- Component 输入不固定，导致每次评估不可比。
- 为了局部 latency/cost 变好，牺牲了 component quality。
