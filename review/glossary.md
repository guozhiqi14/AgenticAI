...
让多个 agent 扮演不同角色并协作完成任务。每个 agent 通常有自己的 role prompt、tools、输入输出格式和责任边界，例如 researcher、designer、writer、critic、tester、editor。

## Role Decomposition

把复杂任务按角色拆分，而不是只按步骤拆分。例如一个 marketing workflow 可以拆成 researcher、graphic designer、writer 和 manager。Multi-agent workflow 通常依赖清晰的 role decomposition。

## Manager Agent

负责规划、委派、协调和 review 的 agent。它可以把 specialist agents 当作可调用对象来分派任务，类似 planning pattern 中 planner 选择 tools。

## Specialist Agent

负责某个清晰子任务的 agent，例如 researcher、statistician、graphic designer、writer 或 editor。Specialist agent 应该有明确职责、工具权限、输入输出契约和 eval 标准。

## Communication Pattern

Multi-agent system 中 agents 之间交换信息和协调工作的方式。常见模式包括 linear handoff、hierarchical/manager coordination、deeper hierarchy、all-to-all 等。通信模式会直接影响 debug、latency、cost、可控性和最终质量。

## Linear Handoff

一种 multi-agent communication pattern，让 agents 按固定顺序传递中间结果，例如 researcher -> designer -> writer。它简单、可控、好 debug，但上游错误容易被下游放大。

## Hierarchical Communication Pattern

一种由 manager agent 协调多个 specialist agents 的通信模式。Specialist 通常把结果回传给 manager，再由 manager 决定下一步和最终整合。

## Deeper Hierarchy

多层级 multi-agent 结构，specialist agent 下面还可以继续调用 sub-agents。它能表达更复杂分工，但会增加 trace、latency、cost 和责任定位难度。

## All-to-All Communication

一种任意 agent 都可以和任意 agent 通信的模式。它适合探索和创意协作，但很难预测、控制和 debug，需要明确 stopping condition。
